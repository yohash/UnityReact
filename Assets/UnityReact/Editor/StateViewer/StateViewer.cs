...
namespace Yohash.React.Editor
{
  public partial class StateViewer : EditorWindow
  {
    ...
    private void populateState()
    {
      ...
      if (state == null) {
        Debug.LogError("State instance is null.");
        return;
      }

      var containers = new List<StateContainer>();

      var fields = state.GetType().GetFields(BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.Public);
      foreach (var field in fields) {
        if (typeof(StateContainer).IsAssignableFrom(field.FieldType)) {
          var container = field.GetValue(state) as StateContainer;
          if (container != null) {
      ...
      var allStateContainers = containers.OrderBy(sc => sc.GetType().Name).ToList();
